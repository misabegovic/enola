...
`)
	ff := extractStimulusBindings(repo, "app/views/things/show.html.erb", src)
	if len(ff) != 3 {
...
func TestStimulusBindings_FailClosed(t *testing.T) {
	...
	if got := extractStimulusBindings(repo, "app/views/x.html.erb", interpolated); len(got) != 0 {
	...
	if got := extractStimulusBindings(repo, "app/views/x.html.slim", slim); len(got) != 0 {
		t.Errorf("only .html.erb is in scope for this pass, got %+v", got)
	}

	namespaced := []byte(`<div data-controller="users--date-picker"></div>`)
	got := extractStimulusBindings(repo, "app/views/y.html.erb", namespaced)
	if len(got) != 1 {
