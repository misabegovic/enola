...
// step from a governing page to the code it governs. On the brain that is 1,828
// anchors and 451 relations — 2,279 edges invisible to every walker — while
...
func TestAnAnchorCarriesAGraphRelation(t *testing.T) {
	page := &PageIntent{Page: &PageDecl{Type: "decision",
		Anchors: []PageAnchor{{Repo: "teamtailor", Path: "app/models/company.rb"}}}}
	for _, f := range CompilePageFacts(page, "wiki/p.md") {
		...
		if got := f.Relations[0].Target; got != "teamtailor/app/models/company.rb" {
